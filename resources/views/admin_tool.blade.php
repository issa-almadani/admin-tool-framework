...
<script src="{{ URL::asset('js/admin/jquery-3.6.0.js') }}"></script> 
<link rel="stylesheet" href="{{ URL::asset('css/admin/jquery-ui.css') }}">

<script>
    // Name of variable must be exactly like the filter name
    // Variables imported from backend
    var filter1 = {!! json_encode($filter1) !!};
    var filter2 = {!! json_encode($filter2) !!};
    var filter3 = {!! json_encode($filter3) !!};

    var filters = {!! json_encode($filters) !!};
    var sortOrder = {!! json_encode($sortOrders) !!};

    var originalData = {!! json_encode($originalData) !!}
